  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    axios.post(`${import.meta.env.VITE_APP_API_HOST}/Lotes`, payload, {
      ...
      .then(() => {
        ...
        fetchLotes();
        setFormData({ nome: "", custoTotal: "", dataCompra: new Date().toISOString().split('T')[0] });
      })
      .catch((error) => {
          ...
          <DialogContent className="sm:max-w-md dark:bg-gray-900 border dark:border-gray-800">
            ...
            <form onSubmit={handleCreate} className="space-y-4 mt-4">
              <div className="space-y-2">
                ...
                <Input id="nome" required value={formData.nome} onChange={(e) => setFormData({ ...formData, nome: e.target.value })} placeholder="Ex: Lote Malange 01/26" className="dark:bg-gray-800 dark:border-gray-700 dark:text-white" />
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input id="data" type="date" required value={formData.dataCompra} onChange={(e) => setFormData({ ...formData, dataCompra: e.target.value })} className="dark:bg-gray-800 dark:border-gray-700 dark:text-white" />
              </div>
              ...
              {loading ? (
                ...
                lotes.map((lote) => (
                  <tr key={lote.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    <td className="px-6 py-4 font-medium text-gray-900 dark:text-white">{lote.nome}</td>
                    <td className="px-6 py-4">{formatDate(lote.dataCompra)}</td>
